...
public class Autonomous
{
	...
	private void autoDriveVision(double tolerance, int lowerLimit){
		...
		while(!reached && offset != 0.0 && isAutoActive()){
			...
			else if((offset < lowerLimit) && (angle < 43.5)){
				Util.consoleLog("Below 43.5");
				Util.consoleLog(" Left motor =%f rightmotor =%f", Devices.LRCanTalon.get(), Devices.RRCanTalon.get()
				);
				Devices.robotDrive.tankDrive(0.4, 0.3);
				reached = false;
			}
		...
		// if(angle > 0){
		// 	Devices.robotDrive.tankDrive(0, -0.2);
		// 	Timer.delay(0.25);
		// }
		// else if(angle < 0) {
		// 	Timer.delay(1);
		// 	Devices.robotDrive.tankDrive(-0.2, 0);
		// 	Timer.delay(0.25);
		// }
		//Timer.delay(1);
		Timer.delay(0.20);
